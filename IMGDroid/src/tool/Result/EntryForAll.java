...
public class EntryForAll { // Entry
	...
	int lines;
	private resulttoexcel excel; // Excel
	...
	public ArrayList<String> getApkFilespath() { // Get APK files, return APK name
		// list
	...
	public void AnalyzeAll(ArrayList<String> selectedApkList) throws IOException, XmlPullParserException {
		...
		for (int i = 0; i < selectedApkList.size(); i++) {
			...
			apkFileLocation = selectedApkList.get(i).toString();
			...
			excel.addOneLine2Excel(curAppName, analysis,  i + 1);
			G.v();
			G.reset();
		}
	...
	public Object[] getResult(int i) {// Get results
		...
		try {
			...
			Sheet sheet = book.getSheet(resulttoexcel.SheetName);
